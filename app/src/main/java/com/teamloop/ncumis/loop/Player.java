...
public class Player
{
     ...
     */
    public Player(SScore score, UserTempo userTempo, Context context, boolean playNotes) throws PlayerException
    {
        mediaPlayerState = MediaPlayerState.Null;
        this.userTempo = userTempo;
        this.playNotes = playNotes;
        try {
            Log.d(TAG,"Enter Player Constructor");
            playData = new PlayData(score, userTempo);
            tempoType = TempoType.absolute;
    ...
    public void pause()
    {
        if (state == State.Started) {
            if (mediaPlayer != null) {
                mediaPlayer.reset(); // tried stop, but after stop prepare just throws an exception and we cannot start again
                mediaPlayerState = MediaPlayerState.Idle;
            }
            dispatcher.stop();
            currentBar = dispatcher.currentBar();
    ...
    public void resume()
    {
        if (state() == State.Paused)
            startAt(currentBar, true/*countIn*/);
    }
    ...
    private void scaleMidiFileTempo()
    {
        if (midiFilePath != null) {
            if (tempoType == TempoType.scaled)
                ...
                PlayData.scaleMIDIFileTempo(midiFilePath, userTempo.getUserTempo()/kDefaultTempoBPM);
        }
    }
